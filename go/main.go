...
func getUserIDOnly(r *http.Request) (v int64, errCode int, errMsg string) {
	session := getSession(r)
	...
	return 0, http.StatusInternalServerError, "db error"
}
...
func getUserSimpleByID(_ sqlx.Queryer, userID int64) (userSimple UserSimple, err error) {
	user := User{}
	err = sqlx.Get(dbx, &user, "SELECT id, account_name, num_sell_items FROM `users` WHERE `id` = ?", userID)
	if err != nil {
	...
	itemsCount := 0
	{
		...
		if itemID > 0 && createdAt > 0 {
			...
			sqlStr, params, err = sqlx.In(
				"SELECT i.id, i.seller_id, i.status, i.name, i.price, i.image_name, i.category_id, i.created_at, u.account_name, u.num_sell_items FROM `items` i JOIN `users` u ON i.seller_id = u.id WHERE i.`status` IN (?,?) AND i.category_id IN (?) AND (i.`created_at` < ?  OR (i.`created_at` <= ? AND i.`id` < ?)) ORDER BY i.`created_at` DESC, i.`id` DESC LIMIT ?",
				ItemStatusOnSale,
				ItemStatusSoldOut,
				categoryIDs,
				time.Unix(createdAt, 0),
			...
			// 1st page
			sqlStr, params, err = sqlx.In(
				"SELECT i.id, i.seller_id, i.status, i.name, i.price, i.image_name, i.category_id, i.created_at, u.account_name, u.num_sell_items FROM `items` i JOIN `users` u ON i.seller_id = u.id WHERE i.`status` IN (?,?) AND i.category_id IN (?) ORDER BY i.created_at DESC, i.id DESC LIMIT ?",
				ItemStatusOnSale,
				ItemStatusSoldOut,
				categoryIDs,
				ItemsPerPage+1)
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	userID, errCode, errMsg := getUserIDOnly(r)
	if errMsg != "" {
	...
	var apiErrs error
	{
		defer rows.Close()
		for rows.Next() {
			// id, seller_id, (seller), buyer_id, (buyer), status, name, price, description, (image_name)image_url, category_id, (category), transaction_evidence_id, transaction_evidence_status, shipping_status, (Unix())created_at
			// id, seller_id, buyer_id, status, name, price, description, (image_name)image_url, category_id, (Unix())created_at, s.name, s.num_, b.name?, b.num_?, transaction_evidence_id, transaction_evidence_status, shippingReserveID
			i := itemCount
			var imageName string
			var t time.Time
			var bName string
			...
			if itemDetails[i].BuyerID != 0 {
				itemDetails[i].Buyer = &UserSimple{
					ID:           itemDetails[i].BuyerID,
					AccountName:  bName,
					NumSellItems: bNum,
				}
			}
			category, _ := getCategoryByID(dbx, itemDetails[i].CategoryID)
			itemDetails[i].Category = &category
			if tID > 0 {
	...
	hasNext := false
	if itemCount > TransactionsPerPage {
		hasNext = true
		itemDetails = itemDetails[0:TransactionsPerPage]
	} else {
		itemDetails = itemDetails[0:itemCount]
	}
...
func postItemEdit(w http.ResponseWriter, r *http.Request) {
	...
	sellerID, errCode, errMsg := getUserIDOnly(r)
	if errMsg != "" {
		outputErrorMsg(w, errCode, errMsg)
		return
	}
...
func postComplete(w http.ResponseWriter, r *http.Request) {
	...
	buyerID, errCode, errMsg := getUserIDOnly(r)
	if errMsg != "" {
		outputErrorMsg(w, errCode, errMsg)
		return
	}
